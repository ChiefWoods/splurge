...
export async function validateProgramIx(
  ...
  return allowedIxs.some(async (ixName) => {
    ...
    const hash = await crypto.subtle.digest("SHA-256", Buffer.from(`global:${ixName}`));
...
export async function sendTx(transaction: string): Promise<SendTransactionResponse> {
  const res = await fetch(`${process.env.GATEWAY_URL}${process.env.GATEWAY_API}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
