    ...
    <li className="flex flex-col gap-y-4">
      <div className="flex w-full justify-between gap-2">
        <div className="flex items-center justify-start gap-x-2">
          <Image
            src={shopper.data.image}
            alt={shopper.data.name}
            width={0}
            height={0}
            className="aspect-square size-6 rounded-full border md:size-10"
          />
          <div className="flex flex-wrap items-center gap-x-2">
            <h3 className="text-base md:text-xl">{shopper.data.name}</h3>
            <p className="text-muted text-xs md:text-sm">{truncateAddress(shopper.data.address)}</p>
          </div>
        </div>
      ...
      <p className="text-sm md:text-base">{review.data.text}</p>
    </li>
  );
}
