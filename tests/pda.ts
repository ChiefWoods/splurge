...
export function getConfigPda() {
  return PublicKey.findProgramAddressSync([Buffer.from("config")], SPLURGE_PROGRAM_ID)[0];
}

export function getTreasuryPda() {
  return PublicKey.findProgramAddressSync([Buffer.from("treasury")], SPLURGE_PROGRAM_ID)[0];
}
...
export function getOrderPda(shopperPda: PublicKey, itemPda: PublicKey, timestamp: BN) {
  return PublicKey.findProgramAddressSync(
    [
      Buffer.from("order"),
      shopperPda.toBuffer(),
